 ...
 */
function isDomainMatch(licenseDomain) {
...
export async function verifyLicenseKey(licenseKey) {
    try {
        ...
        if (!cleanKey.startsWith("NEAT-"))
            return { valid: false, reason: "Key must start with \"NEAT-\" prefix" };
        const rest = cleanKey.slice(5); // Remove "NEAT-" prefix
        const dotIndex = rest.indexOf(".");
        if (dotIndex === -1)
            return { valid: false, reason: "Invalid key format: missing separator" };
        const payloadB64 = rest.slice(0, dotIndex);
        const signatureB64 = rest.slice(dotIndex + 1);
        if (!payloadB64 || !signatureB64)
            return { valid: false, reason: "Invalid key format: empty payload or signature" };
        // Decode payload
        ...
        if (!payload.domain || typeof payload.domain !== "string")
            return { valid: false, reason: "Invalid payload: missing domain" };
        // Check domain match
        ...
        // Import public key
        const publicKey = await crypto.subtle.importKey("jwk", NEAT_PUBLIC_KEY_JWK, { name: "ECDSA", namedCurve: "P-256" }, false, ["verify"]);
        // Verify signature over the raw payload bytes (UTF-8 JSON)
        const valid = await crypto.subtle.verify({ name: "ECDSA", hash: "SHA-256" }, publicKey, signatureBuffer, payloadBuffer);
 ...
 */
export function _getPublicKeyJwk() {
